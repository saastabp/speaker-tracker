...
def _org(org_type_id: int, name: str = "PWN", **kw) -> OrganizationInput:
    return OrganizationInput(organization_type_id=org_type_id, name=name, **kw)


def test_create_and_get(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    org_id = orgs.create_organization(conn, user_id, _org(tid, why_it_fits="great fit"))
...
def test_list_is_name_ordered_and_carries_why_it_fits(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    orgs.create_organization(conn, user_id, _org(tid, "Bravo", why_it_fits="fits B"))
    orgs.create_organization(conn, user_id, _org(tid, "Alpha", why_it_fits="fits A"))
...
def test_duplicate_live_name_conflicts(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    orgs.create_organization(conn, user_id, _org(tid, "PWN"))
...
def test_name_reusable_after_soft_delete(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    org_id = orgs.create_organization(conn, user_id, _org(tid, "PWN"))
    assert orgs.soft_delete_organization(conn, user_id, org_id) is True
    orgs.create_organization(conn, user_id, _org(tid, "PWN"))  # no Conflict


def test_unknown_type_is_invalid_input(seeded_db) -> None:
    conn, user_id, _, _ = seeded_db
    with pytest.raises(errors.InvalidInput):
        orgs.create_organization(conn, user_id, _org(999999, "X"))


def test_update_replaces_fields(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    org_id = orgs.create_organization(conn, user_id, _org(tid, "PWN", location="Kauai"))
    assert (
        orgs.update_organization(conn, user_id, org_id, _org(tid, "PWN", location="Oahu")) is True
    )
...
def test_update_missing_returns_false(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    assert orgs.update_organization(conn, user_id, 999, _org(tid, "X")) is False


def test_update_to_duplicate_name_conflicts(seeded_db) -> None:
    conn, user_id, tid, _ = seeded_db
    orgs.create_organization(conn, user_id, _org(tid, "Alpha"))
    bravo = orgs.create_organization(conn, user_id, _org(tid, "Bravo"))
    with pytest.raises(errors.Conflict):
        orgs.update_organization(conn, user_id, bravo, _org(tid, "Alpha"))
